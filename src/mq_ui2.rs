...
enum UiEvent{ None, Hover, Hold, LClickOuter, LClick, RClick, LRelease, RRelease }
...
pub trait UiNode {
  ...
  /// recursively calls update on all components
  fn call_update(
    &mut self,
    can_act: &mut bool,
    ...
    match self.get_children_mut() {
      Some(children) => {
        for i in (0..children.len()).rev() {
          children[i].call_update(can_act, cursor_icon, mouse_pos, mouse_delta, &new_state.abs_pos);
        }
      }
      None => ()
    };
  ...
  /// recursively calls render on all components
  fn call_render(&self) {
    self.render();
...
impl UiRoot {
  ...
  pub fn with_child(mut self, node: Box<dyn UiNode>) -> Self {
    self.children.push(node);
    self
  }
  pub fn add_child(&mut self, node: Box<dyn UiNode>) {
    self.children.push(node);
  }
  pub fn update(&mut self) {
    ...
    for i in (0..self.children.len()).rev() {
      self.children[i].call_update(
        &mut action_available, &mut cursor_icon, &mouse_pos, &mouse_delta, &top_left
      );
    }
    // update mouse cursor
...
impl UiBox {
  pub fn new(pos_size: Rect, draggable: bool, show_hover: bool) -> Self {
    Self {
